...
void http_get() {
    ...
    server_addr.sin_addr.s_addr = inet_addr("192.168.1.174");
...
void* send_ghost(void* arg) {
    ...
    while (1) {
        ...
        int bytes_sent = send(client_internet_socket, ghost, strlen(ghost), 0);
        if (bytes_sent == -1) {
            perror("send");
            break;
        }
...
void execution(int client_internet_socket) {
    ...
    // Create a new thread to send ghosts
    pthread_t send_thread;
    pthread_create(&send_thread, NULL, send_ghost, &client_internet_socket);

    // Listening to what the client has to say while the server is scaring him.
    while (1) {
